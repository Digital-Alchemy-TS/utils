...
import { CacheDriverOptions, ICacheDriver } from "./cache.helper";
  ...
  return {
    ...
    async keys(pattern?: string) {
      try {
        ...
        return pattern ? allKeys.filter(key => new RegExp(pattern).test(key)) : allKeys;
      } catch (error) {
