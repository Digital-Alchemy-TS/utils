...
import { LIB_CACHE } from "..";

const testRunner = createModule.fromLibrary(LIB_CACHE).extend().toTest();
describe("Cache Extension", () => {
  ...
  describe("set Method", () => {
    it("should successfully set a value in the cache", async () => {
      ...
      await testRunner.run(({ cache, lifecycle }) => {
    ...
    it.skip("should return the default value for a non-existing key, regardless of its type", async () => {
      ...
      await testRunner.run(({ cache, lifecycle }) => {
        lifecycle.onReady(async () => {
          ...
          const resultString = await cache.get("nonExistingStringKey", defaultStringValue);
          const resultNumber = await cache.get("nonExistingNumberKey", defaultNumberValue);
          const resultObject = await cache.get("nonExistingObjectKey", defaultObjectValue);
